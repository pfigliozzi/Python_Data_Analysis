...
def find_frames_particle_number_diff_rolling_mode(df, window):
	...
	particles_per_frame = df.groupby('frame')
	rolling_mode = df.groupby('frame').apply(mode_for_rolling)

	# Add values of to the rolling mode to the ends with smaller windows
	for frame in window/2:
		rolling_mode.iloc[frame] = rolling_mode.iloc[:window].mode()
		rolling_mode.iloc[-(frame+1)] = rolling_mode.iloc[-window:].mode()
